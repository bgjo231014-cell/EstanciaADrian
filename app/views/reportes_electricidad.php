...
<div class="container mt-4">
    <h3 class="text-center text-warning mb-4">
        Reporte de Consumo de Electricidad
    </h3>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped text-center">
                        ...
                        <?php foreach ($datos as $row): ?>
                            <tr>
                                <td><?= $row['anio'] ?></td>
                                <td><?= number_format($row['total_kw'], 2) ?></td>
    ...
    <div class="card shadow mb-5">
        <div class="card-header bg-dark text-white">
            <h5 class="mb-0">Gráfica: Consumo de Electricidad y Costo</h5>
        </div>
        <div class="card-body">
            <canvas id="graficaElectricidad" height="120"></canvas>
        </div>
    </div>
</div>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const kw = datos.map(d => d.total_kw);
    const costos = datos.map(d => d.total_costo);

    const ctx = document.getElementById("graficaElectricidad");
